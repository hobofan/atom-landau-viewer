...
var JsCadViewerView = (function () {
  ...
  function JsCadViewerView(pathname) {
    ...
    var tree = this.tree = document.createElement('div');
    tree.id = 'tree-' + this.fileIdentifier;
    tree.style.height = '200px';
    tree.style['overflow-y'] = 'scroll';
    var table = document.createElement('table');
    var treeRoot = this.treeRoot = document.createElement('tbody');
    this.element.appendChild(tree);
    tree.appendChild(table);
    table.appendChild(treeRoot);
    ...
    var nodeBinary = '/Users/hobofan/stuff/snappy-reprap/node_modules/.bin/babel-node'; // TODO: atom config

    this.render = function () {
      var processor = this.processor;
      var element = this.element;
      function execute(command, callback){
          // console.log("DEBUG_COMMAND", command);
          exec(command, function(error, stdout, stderr){
            // console.log("DEBUG_STDOUT", stdout);
            // console.log("DEBUG_STDERR", stderr);
            callback(stdout);
            if (stderr) {
      ...
      var packagerUrl = 'http://localhost:1938';
      ...
      function getRendered(optionsPackets, callback) {
        // var converted = [];
        function recurse(finished, remaining) {
          if (remaining.length === 0) {
            ...
            execute(renderCommand(remaining.shift()), function(mainJson){
              var cvrt = CSG.cube().fromJSON(JSON.parse(mainJson.trim()));
              finished.push(cvrt);
        ...
        // optionsPackets.forEach(function(options) {
        //   execute(renderCommand(options), function(mainJson){
        //     var cvrt = CSG.cube().fromJSON(JSON.parse(mainJson.trim()));
        //     converted.push(cvrt);
        //   });
        // });
      ...
      var mainOpts = { module_path: pathname, pos: treePosView };
      var childOpts = [];
      Array.from(Array(treeSize[treePosView] || 0).keys()).forEach(function (i) {
        childOpts.push({ module_path: pathname, pos: treePosView.concat([i]) });
      });

      // var child1Opts = { module_path: pathname, pos: [0, 0] };
      // var child2Opts = { module_path: pathname, pos: [0, 1] };
      // var child3Opts = { module_path: pathname, pos: [0, 2] };

      // execute(renderCommand(mainOpts), function(mainJson){
      //   var mainConverted = CSG.cube().fromJSON(JSON.parse(mainJson.trim()));
      // execute(renderCommand(child1Opts), function(child1Json){
      //   var child1Converted = CSG.cube().fromJSON(JSON.parse(child1Json.trim()));
      // execute(renderCommand(child2Opts), function(child2Json){
      //   var child2Converted = CSG.cube().fromJSON(JSON.parse(child2Json.trim()));
      // execute(renderCommand(child3Opts), function(child3Json){
      //   var child3Converted = CSG.cube().fromJSON(JSON.parse(child3Json.trim()));
      getRendered([mainOpts].concat(childOpts), function(converted){
        processor.setCurrentObjects(converted);
    ...
    this.build_tree = function (renderFn) {
      function execute(command, callback){
          exec(command, function(error, stdout, stderr){ callback(stdout); });
      };

      var scriptTemplateTree = "\
        var obj = require('PATHNAME');\
        var result = obj.build().tree();\
        console.log(JSON.stringify(result));\
      ";
      var renderedTemplateTree = scriptTemplateTree.replace('PATHNAME', pathname);

      execute(nodeBinary + ' -e "' + renderedTemplateTree + '"',
        function(output){
          var json = JSON.parse(output.trim());

          function addElement(treeRoot, name, pos) {
          ...
          function traverse(tree, pos) {
            addElement(treeRoot, Array(pos.length).join('-') + tree.elementName, pos);
            if (tree.children) {
  ...
  JsCadViewerView.prototype.loadDependencies = function () {
    ...
    async.eachSeries(jsDependencies, function (scriptName, next) {
      ...
      setTimeout(function () {
        // console.log('render');
        self.render();
        self.build_tree(self.render.bind(self));
      }, 250);
